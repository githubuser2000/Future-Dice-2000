...
#                for i,el in enumerate(elo):
        ...
        for i,oneOf2 in enumerate(result):
            if  type(oneOf2) in [tuple,list] and len(oneOf2) in [3,4]:
                ...
                elif type(erstwuerfe) in [tuple,list] and len(oneOf2) == 3:
                ...
                for k,erstwuerfe in enumerate(oneOf2):
                    if  len(erstwuerfe) in [3,4]:
                        self.wuerfe += 1
                        if type(erstwuerfe) in [tuple,list] and len(erstwuerfe) == 4:
                        ...
                        elif type(erstwuerfe) in [tuple,list] and len(erstwuerfe) == 3:
    ...
    def wuerfelErstellen(self):
        ...
        if not self.wuerfelrestellt:
            ...
            if not gezinkt:
                ...
                self.dice = libdice.dice(['dicegui',augen.property("text"),'gewicht',('-' if reverse.property("position")==1 else '' )+LRad.property("text"),n.property("text"),x.property("text"),y.property("text"),('-' if reverse2.property("position")==1 else '' )+LRad2.property("text"),n2.property("text"),x2.property("text"),y2.property("text")] + Lists,int(wuerfe.property("text")), True if uniq.property("position")==1 else False,planesNames.property("text") if planesNames.property("sett") else "",nega_.property("checked"),medi_.property("checked"))
